...
uint16_t sqrt_long( uint32_t sqr )
{
	...
	int16_t iterations = 5 ;  // just a couple more iterations produces near exact result
...
{
	uint16_t magnitude , half_mag ;
	magnitude = vector2_mag( input[0] , input[1] ) ;
	half_mag = magnitude/2 ;
	if ( half_mag > 0 )
	{
		result[0] = __builtin_divsd( __builtin_mulss( RMAX/2 , input[0] ) , half_mag ) ;
		result[1] = __builtin_divsd( __builtin_mulss( RMAX/2 , input[1] ) , half_mag ) ;
	}
	else
	{
		result[0]=result[1]=0;
...
uint16_t vector3_normalize( int16_t result[] , int16_t input[] )
{
	...
	if ( half_mag > 0 )
	{
		result[0] = __builtin_divsd( __builtin_mulss( RMAX/2 , input[0] ) , half_mag ) ;
		result[1] = __builtin_divsd( __builtin_mulss( RMAX/2 , input[1] ) , half_mag ) ;
		result[2] = __builtin_divsd( __builtin_mulss( RMAX/2 , input[2] ) , half_mag ) ;
	}
	else
	{
...
int16_t Cross2D(fractional * src1, fractional * src2)
{
