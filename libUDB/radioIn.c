...
int16_t pitchDownPulses = 0 ;
int16_t pitchDownNoisePulses = 0 ;
int16_t rollOffsetPulses = 0 ;
int16_t rollOffsetNoisePulses = 0 ;
...
#if ( RTL_PITCH_DOWN_INPUT_CHANNEL > 0 )
	if (RTL_PITCH_DOWN_INPUT_CHANNEL == index)
	{
		if ((pwm > RTL_PITCH_DOWN_INPUT_MIN) && (pwm < RTL_PITCH_DOWN_INPUT_MAX))
		{
			pitchDownPulses++;
		}
		else
		{
			pitchDownNoisePulses++;
		}
	}
#endif // RTL_PITCH_DOWN_INPUT_CHANNEL

#if ( ROLL_OFFSET_INPUT_CHANNEL > 0 ) 
	if (ROLL_OFFSET_INPUT_CHANNEL  == index)
	{
		if ((pwm > ROLL_OFFSET_INPUT_MIN) && (pwm < ROLL_OFFSET_INPUT_MAX))
		{
			rollOffsetPulses++;
		}
		else
		{
			rollOffsetNoisePulses++;
		}
	}
#endif // ROLL_OFFSET_INPUT_CHANNEL
