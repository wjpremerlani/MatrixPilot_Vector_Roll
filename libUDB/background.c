...
void ignore_radio ( void )
{
	// 20Hz servicing of radio link
	if ((udb_heartbeat_counter % (HEARTBEAT_HZ/20)) == 1)
	{
		// turn off the radio
...
void test_arduino_link ( void )
{
	...
	if ((udb_heartbeat_counter % (HEARTBEAT_HZ/20)) == 1)
	{
		// check to see if at least one valid pulse has been received,
		// and also that the noise rate has not been exceeded
		if ((pitchDownPulses == 0) || (pitchDownNoisePulses > MAX_NOISE_RATE)
				|| (rollOffsetPulses == 0) || ( rollOffsetNoisePulses > MAX_NOISE_RATE))
		{
			udb_flags._.arduino_on = 0;
			LED_BLUE = LED_OFF;
			pitchDownNoisePulses = 0; // reset count of noise pulses
			rollOffsetNoisePulses = 0 ;
		}
		else
		{
			udb_flags._.arduino_on = 1;
			LED_BLUE = LED_ON;
		}
		pitchDownPulses = 0;
		rollOffsetPulses = 0;
