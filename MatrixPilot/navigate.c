...
#ifdef YAWKP_AILERON
#warning "YAWKP_AILERON is no longer used, define MAXIMUM_LATERAL_ACCELERATION instead"
#endif // YAWKP_AILERON

const uint16_t acceleration_kp = 2.0*(MAXIMUM_LATERAL_ACCELERATION/4.0)*RMAX ;
...
#else 
//	const uint16_t yawkpail = (uint16_t) (YAWKP_AILERON*RMAX) ; // no longer used
	const uint16_t yawkpail = 0 ;
